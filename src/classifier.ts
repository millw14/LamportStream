...
export function classify(scout: ScoutResult): ClassificationResult {
  ...
  const useSignatureHotBands = referenceDensity >= ULTRA_DENSE_SIGNATURE_THRESHOLD;
  ...
  if (!scout.hasMore && scout.scoutCount < 50) {
    return { density: WalletDensity.Sparse, minChunks: 4, maxChunks: 8, concurrency: 8, useSignatureHotBands };
  }
  ...
  if (estimatedTotalTx <= 3000 && referenceDensity < 0.01) {
    return { density: WalletDensity.Medium, minChunks: 16, maxChunks: 32, concurrency: 24, useSignatureHotBands };
  }

  return { density: WalletDensity.Dense, minChunks: 48, maxChunks: 64, concurrency: 40, useSignatureHotBands };
}
